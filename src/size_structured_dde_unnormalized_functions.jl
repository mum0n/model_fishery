##  NOTE: this is out of syncand needs to be revisited ... look at normalised version
...
@model function size_structured_dde_turing( S, kmu, tspan, prob,  nS,
    ...
    # K ~ filldist( truncated( Normal( kmu, kmu*0.2), kmu/1000.0, kmu*1000.0), nS )  # kmu is max of a multiyear group , serves as upper bound for all
    ...
    q ~ filldist( truncated( Normal( 1.0, 0.1),  0.2, 5.0), nS )
    ...
    # b ~ filldist( truncated( Normal(5.0, 0.1), 0.01, 100.0), 2 )
    b ~ filldist( truncated(Chisq( 3 ), 0.1, 10),  2 )   # centered on 1; plot(x->pdf(Chisq(7), x), xlim=(0,10)) # mode of 5

    # background mortality
    d ~ filldist( truncated( Normal(0.2, 0.1), 0.01, 1.0), nS )

    # excess mortality due to habitat (second order)
    d2 ~ filldist( truncated( Normal(0.2, 0.1), 0.01, 1.0), nS )

    # transition rates
    v ~ filldist( truncated( Normal(0.9, 0.1), 0.01, 1.0), 4 )

    # initial conditions
    u0 ~ filldist( truncated( Normal(0.5, 0.3), 0.01, 1.0), nS )
